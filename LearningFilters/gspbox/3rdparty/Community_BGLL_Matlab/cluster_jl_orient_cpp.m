...
function [COMTY ending] = cluster_jl_orient_cpp(M,s,self,debug,verbose)
...
[COM Niter] = jl_clust_orient(M,debug);
...
if (s == 1)
    ...
    while 1
        ...
        Nnew = size(Mnew);
        Nnew = Nnew(1);
        [COMt e] = cluster_jl_orient_cpp(Mnew,0,1,debug);
        if (e ~= 1)
            COMfull = zeros(1,N);
            COMcur = COMt.COM{1};
            for p=1:Ncom
